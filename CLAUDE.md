...
- A **public tools hub** (`/tools`) — a login-free, Supabase-free landing page linked from the portfolio nav that lists every tool usable **without an account** (excludes the admin-only tools `/design`, `/studio`, `/shopee`, which stay gated inside the forum)
...
├── thiepcuoi/index.html      # Personal wedding invitation (not a tool — not listed on /tools)
...
| `/thiepcuoi` | Personal wedding invitation (Ngọc Huy & Hương Giang, 27.09.2026) | **Not a tool** — personal page, deliberately absent from `/tools` and the forum grid. Single self-contained file, no CDN except Google Fonts (Cormorant Garamond). The floral arch is inline SVG composed at load from a botanical `<defs>` library via a **seeded** RNG, so the layout is identical on every visit. The script face (**VL Great Vibes**) is inlined as a base64 woff2 because Parisienne — both the local `.ttf` and the Google Fonts build — is missing 48/74 Vietnamese precomposed diacritics. Gift button opens a dialog of **VietQR** codes (EMVCo payload + CRC-16) generated by `tools/make_qr.py` in the source repo. Upstream source of truth: private repo `hulecorp-tech/wedding-invitation` — edit there, then copy `index.html` here |
